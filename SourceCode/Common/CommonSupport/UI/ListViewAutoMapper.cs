...
namespace Nitro.Framework.Goliath.FrontEnd
{
    ...
    public class ListViewAutoMapper
    {
        ...
        volatile Orders _order = Orders.Normal;
        public Orders Order
        {
            get { return _order; }
            set { _order = value; }
        ...
        void DoUpdateUI()
        {
            ...
            {// Lock prior to accessing the source.
                ...
                if (ReflectionHelper.IsTypeImplementingInterface(source.GetType(), typeof(IEnumerable)))
                {
                    ...
                    if (_order == Orders.Reversed)
                    {
                        List<object> itemsReversed = new List<object>();
                        foreach (object item in enumerable)
                        {
                            itemsReversed.Insert(0, item);
                        }

                        enumerable = itemsReversed;
                    }
