...
extern bool e_smartValueUpdate = true;
...
// Result format : int& orderTicket, int& operationID, double& stopLoss, double takeProfit, double targetOpenPrice
...
void HandleNewOrderRequests()
{
   ...
   while(ParseRequestNewOrder(symbol, volume, prefferedPrice, slippage, takeProfit, stopLoss, orderType, operationId, comment))
   {
      Trace("OpeningOrder volume[" + volume + "] prefferedPrice[" + prefferedPrice + "] slippage[" + slippage + "] takeProfit[" + takeProfit + "] stopLoss[" + stopLoss + "] orderType[" + orderType + "] operationId[" + operationId + "] comment[" + comment + "]", false);
      ...
      if (slippage < 0)
      {// Full ask value.
        slippage = MarketInfo(symbol, MODE_ASK) / MarketInfo(symbol, MODE_POINT);
      }
      ...
      {// Pending order.
         orderTicket = OrderSend(symbol, orderType, volume, prefferedPrice, slippage, stopLoss, takeProfit, comment, GetNextUniqueMagicNumer(), 0, Green);
         Trace(">>" + orderType + " " + volume+ " " + prefferedPrice+ " " + slippage + " " + stopLoss+ " " + takeProfit, false);
         sent = true;
      }
      else
      {// Market order.
         double requestPrice = MarketInfo(symbol, MODE_ASK);
         if (orderType == OP_SELL)
         {
            requestPrice = MarketInfo(symbol, MODE_BID);
         }
      
         double actualSlippage = slippage * MarketInfo(symbol, MODE_POINT);
      
         if (actualSlippage < MathAbs(requestPrice - prefferedPrice))
         {// Price slipped too much.
            operationResultMessage = "Slippage criteria not met, price desired: " + prefferedPrice + ", actual: " + requestPrice + ", actual slippage: " + actualSlippage;
            Trace("Slippage occured, price desired: " + prefferedPrice + ", actual: " + requestPrice + ", actual slippage: " + actualSlippage, false);
         }
         else
         { 
            double actualSlippageDeduction = MathAbs(requestPrice - prefferedPrice);
            int slippageDeduction = slippageDeduction / MarketInfo(symbol, MODE_POINT);
            
            Trace("SendingOrder volume[" + volume + "] prefferedPrice[" + prefferedPrice + "] requestPrice[" + requestPrice + "] request slippage [" + (slippage /*- slippageDeduction*/) + "] takeProfit[" + takeProfit + "] stopLoss[" + stopLoss + "] orderType[" + orderType + "] operationId[" + operationId + "] comment[" + comment + "]", false);
            orderTicket = OrderSend(symbol, orderType, volume, requestPrice, slippage /*- slippageDeduction*/, stopLoss, takeProfit, comment, GetNextUniqueMagicNumer(), 0, Green);
            sent = true;
         }
      }
...
void HandleCloseOrderRequests()
{
   ...
   while(ParseRequestCloseOrder(orderTicket, operationId , closingVolume, prefferedPrice, slippage))
   {
      Trace("Closing Order ticket[" + orderTicket + "] closingVolume[" + closingVolume + "] prefferedPrice["+ prefferedPrice + "] slippage[" + slippage + "]", false);
...
*/
      ...
      if (slippage < 0)
      {// Full ask value.
         slippage = MarketInfo(OrderSymbol(), MODE_ASK) / MarketInfo(OrderSymbol(), MODE_POINT);
      }

      double actualSlippage = slippage * MarketInfo(OrderSymbol(), MODE_POINT);

      if (slippage < MathAbs(currentClosingPrice - prefferedPrice))
      {// Price slipped too much.
         operationResultMessage = "Slippage criteria not met, price desired: " + prefferedPrice + ", actual:" + currentClosingPrice + ", actual slippage: " + actualSlippage;
         Trace("Slippage occured on closing order, price desired: " + prefferedPrice + ", actual:" + currentClosingPrice, false);
      }
      else
      {
        fullOrderClose = closingVolume == OrderLots();
        
        // TODO: add the additional slippage compensation here...
        if (OrderClose(orderTicket, closingVolume, currentClosingPrice, slippage, Yellow) == false)
...
void SendTradingValues(bool ratesWereRefreshed)
{
   ...
   {// Do the general refresh since the other one was specific request.
      // NOTE: Open, Close etc. arrays are indexed backwards, 0 being the latest bar.
      Quotes(GetExpertId(), Symbol(), -1, Ask, Bid, Open[0], Close[0], Low[0], High[0], Volume[0], TimeCurrent());
      
      if (g_lastDataHistoryPeriod != Time[0])
      {// Send updates of data history every time a new bar is formed.
         SendTradingValuesHistory(-1, 20);
